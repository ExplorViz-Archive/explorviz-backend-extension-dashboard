...
public class LandscapeSerializationHelper {
	 ...
	 *
	 * @throws DocumentSerializationException if the landscape list could not be
	 *                                        parsed.
	 */
	 ...
	 *                                        deserialized to a landscape
	 */
	public List<Landscape> deserializeToList(final String jsonApi) throws DocumentSerializationException {

		final byte[] b = jsonApi.getBytes();
		final JSONAPIDocument<List<Landscape>> landscapeDoc = this.jsonApiConverter.readDocumentCollection(b,
				Landscape.class);
